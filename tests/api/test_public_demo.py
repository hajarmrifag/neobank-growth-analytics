...
def account(client, headers):
    response = client.post(
        "/accounts",
        headers=headers,
        json={"owner_name": "Fictional Demo", "opening_balance_minor": 1000},
    )
    assert response.status_code == 201, response.text
...
def test_public_session_isolation_and_scoped_keys(database_url):
    with TestClient(create_app(database_url, public_demo=True)) as client:
        ...
        assert (
            client.post(
                "/transfers", headers={**a, "Idempotency-Key": "same"}, json=payload
            ).status_code
            == 404
        )
        for headers, src, dst in [(a, a1, a2), (b, b1, b2)]:
            ...
            assert (
                client.post(
                    "/transfers", headers={**headers, "Idempotency-Key": "same"}, json=payload
                ).status_code
                == 201
            )
            assert (
...
def test_public_limits(database_url):
    with TestClient(create_app(database_url, public_demo=True)) as client:
        ...
        assert (
            client.post("/accounts", headers=headers, json={"owner_name": "Demo"}).status_code
            == 429
        )
        ...
        assert (
            client.get(
                "/accounts/00000000-0000-0000-0000-000000000000", headers=headers
            ).status_code
            == 429
        )
...
def test_expiry_cleanup_preserves_local_accounts(database_url):
    ...
    with TestClient(create_app(database_url, public_demo=True)) as client:
        ...
        response = client.post(
            "/transfers",
            headers={**headers, "Idempotency-Key": "expiry"},
            json={"source_account_id": one, "destination_account_id": two, "amount_minor": 10},
        )
        assert response.status_code == 201
        with connect(database_url) as conn:
            conn.execute(
                "UPDATE banking.demo_sessions SET expires_at = clock_timestamp() - INTERVAL '1 second'"
            )
        assert client.get(f"/accounts/{one}", headers=headers).status_code == 401
        with connect(database_url) as conn:
            cleanup(conn)
            assert (
                conn.execute("SELECT count(*) AS n FROM banking.demo_sessions").fetchone()["n"] == 0
            )
            assert (
                conn.execute("SELECT id FROM banking.accounts WHERE id = %s", (one,)).fetchone()
                is None
            )
            assert conn.execute(
                "SELECT id FROM banking.accounts WHERE id = %s", (local_id,)
            ).fetchone()
...
def test_concurrent_account_quota(database_url):
    ...
    with TestClient(app) as client:
        ...
        def create(_):
            return client.post(
                "/accounts", headers=headers, json={"owner_name": "Demo"}
            ).status_code
