...
def account(client, balance=0):
    result = client.post(
        "/accounts", json={"owner_name": "Demo Customer", "opening_balance_minor": balance}
    )
...
@pytest.mark.parametrize(
    ...
    [
        {"owner_name": "  "},
        {"owner_name": "A", "opening_balance_minor": -1},
        {"owner_name": "A", "opening_balance_minor": 0.5},
        {"owner_name": "A", "currency": "USD"},
        {"owner_name": "A", "admin": True},
    ],
)
...
def test_concurrent_spending_cannot_overdraw(client, database_url):
    ...
    results = concurrent_requests(
        database_url, [(body(source, destination, 150), str(uuid4())) for _ in range(10)]
    )
...
def test_database_failure_after_updates_rolls_back_everything(client, database_url):
    ...
    try:
        with pytest.raises(psycopg.errors.RaiseException), connect(database_url) as conn:
