...
def cleanup(conn):
    with conn.transaction():
        ...
        if not ids:
            return
        conn.execute(
            """DELETE FROM banking.transfers WHERE source_account_id IN
            (SELECT id FROM banking.accounts WHERE demo_session_id = ANY(%s))""",
            (ids,),
        )
...
def start_session(conn):
    with conn.transaction():
        ...
        row = conn.execute(
            """INSERT INTO banking.demo_sessions (id, token_hash)
            VALUES (%s, %s) RETURNING expires_at""",
            (uuid4(), sha256(token.encode()).hexdigest()),
        ).fetchone()
...
def authenticate(conn, token):
    if not token or len(token) > 128:
        raise BankingError(
            401, "session_required", "Create a demo session and use its token as a Bearer token"
        )
    with conn.transaction():
        row = conn.execute(
            """SELECT *, window_start <= clock_timestamp() - INTERVAL '1 minute' AS new_window
            FROM banking.demo_sessions WHERE token_hash = %s AND expires_at > clock_timestamp() FOR UPDATE""",
            (sha256(token.encode()).hexdigest(),),
        ).fetchone()
        if not row:
            raise BankingError(
                401, "session_expired", "Invalid or expired demo session; start a new session"
            )
        if row["new_window"]:
            conn.execute(
                "UPDATE banking.demo_sessions SET window_start = clock_timestamp(), request_count = 1 WHERE id = %s",
                (row["id"],),
            )
            ...
            conn.execute(
                "UPDATE banking.demo_sessions SET request_count = request_count + 1 WHERE id = %s",
                (row["id"],),
            )
    return row["id"]
                ...
                return await JSONResponse(
                    {
                        "error": {
                            "code": "body_too_large",
                            "message": "Maximum request body is 4 KB",
                        }
                    },
                    status_code=413,
                )(scope, receive, send)
