...
from banking_api.sandbox import BodyLimitMiddleware, authenticate, cleanup, start_session
...
def create_app(database_url=None, public_demo=None):
    public_demo = (
        public_demo if public_demo is not None else os.environ.get("BANKING_PUBLIC_DEMO") == "1"
    )
    ...
    app = FastAPI(
        lifespan=lifespan,
        title="Neobank Transaction API",
        version="1.0.0",
        description="Portfolio simulation with synthetic funds only. GBP amounts are integer pence. "
        + (
            "First create a session at POST /demo/sessions, then click Authorize and paste its token. Sessions expire after 30 minutes. "
            if public_demo
            else "Local mode: no authentication. "
        )
    ...
    def service(credentials: Annotated[HTTPAuthorizationCredentials | None, Depends(bearer)]):
        with connect(database_url) as conn:
            session_id = (
                authenticate(conn, credentials.credentials if credentials else None)
                if public_demo
                else None
            )
        ...
        return JSONResponse(
            ...
            content={
                "error": {
                    ...
                    "message": "Database temporarily unavailable; retry with the same idempotency key",
                }
            },
        )
    ...
    @app.post(
        ...
        status_code=201,
        responses={
            200: {"description": "Previously committed transfer replayed"},
