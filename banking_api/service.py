...
class BankingService:
        ...
        with self.conn.transaction():
            ...
            if self.session_id:
                count = self.conn.execute(
                    "SELECT count(*) AS n FROM banking.accounts WHERE demo_session_id = %s",
                    (self.session_id,),
                ).fetchone()["n"]
                if count >= 5:
                    raise BankingError(
                        429, "account_limit", "Demo limit: five accounts per session"
                    )
    ...
    def account(self, account_id):
        row = self.conn.execute(
            "SELECT * FROM banking.accounts WHERE id = %s",
            (account_id,),
        ).fetchone()
    ...
    def transfer(self, request, key):
        ...
        with self.conn.transaction():
            ...
            previous = self.conn.execute(
                "SELECT * FROM banking.transfers WHERE idempotency_key = %s",
                (key,),
            ).fetchone()
            if previous:
                ...
                if any(previous[field] != getattr(request, field) for field in fields):
                    raise BankingError(
                        409, "idempotency_conflict", "Key already used for a different transfer"
                    )
                return previous, True

            if self.session_id:
                count = self.conn.execute(
                    """SELECT count(*) AS n FROM banking.transfers t
                    JOIN banking.accounts a ON a.id = t.source_account_id WHERE a.demo_session_id = %s""",
                    (self.session_id,),
                ).fetchone()["n"]
                if count >= 100:
                    raise BankingError(
                        429, "transfer_limit", "Demo limit: 100 transfers per session"
                    )
